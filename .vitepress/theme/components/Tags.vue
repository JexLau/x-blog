<template>
  ...
  <a v-show="!article.frontMatter.home" :href="base + article.regularPath || ''"
    v-for="(article, el) in data[selectTag]" :key="el" class="article">
    <div class="title">
  ...
  <div class="tags">
    <span @click="toggleTag(key)" v-for="(item, key, index) in data" class="tag" :key="index + key">
      {{ key }}
    </span>
  </div>
</template>
